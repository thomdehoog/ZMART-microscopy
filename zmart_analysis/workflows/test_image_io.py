...
class TestSteps(OmeZarrTestCase):
    ...
    def _load_step(name):
        ...
        if not path.is_file():
            # ZMART vendors the reader but not the rare_event_selection steps
            # it was written beside. See ../README.md for what was taken.
            raise unittest.SkipTest(f"{name} is not part of this checkout")
...
class TestEnvironmentBoundary(OmeZarrTestCase):
    ...
    def setUpClass(cls):
        ...
        # This exercises the file_paths boundary through a module-level
        # helper that this repository's engine does not have: it runs steps
        # through its worker pool instead. The mechanism is the same and the
        # entry point is not, so the test is only meaningful with that engine.
        import engine as _engine
        if not hasattr(_engine, "run_in_subprocess"):
            raise unittest.SkipTest(
                "this checkout's engine runs steps through a worker pool, "
                "not engine.run_in_subprocess"
            )
        ...
        cls.step_dir.mkdir()
        shutil.copy(Path(__file__).parent / "_image_io.py", cls.step_dir)
