...
// One link, at the top of `parked/`, so that every option folder beneath it
// resolves its packages the ordinary way with no configuration at all.
const parked = dirname(here);
const theViewers = join(parked, "..", "app", "page", "node_modules");
const link = join(parked, "node_modules");
...
console.log(`parked/node_modules -> ${relative(parked, theViewers)}`);
