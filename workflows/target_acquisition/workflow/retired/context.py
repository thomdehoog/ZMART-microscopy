...
- `Config` is the immutable record of operator inputs, constructed once
  ...
  the machine-local `limits.json` (its `stage.*` constraints); Step 2
  writes the active working envelope to `limits/.../current.json`.
...
class Config:
    ...
    Navigator Expert (preferred); the physical envelope from the
    machine-local `limits.json` (its `stage.*` constraints) is used as
    the safety ceiling. The cfg
    ...
    Z-wide limits always come from the machine-local `limits.json` (its
    `stage.*` constraints; the physical envelope); there is no
    operator-typed override -- focus behaviour
    ...
    # physical envelope from the machine-local `limits.json` (its `stage.*`
    # constraints); a ValueError is raised if any value falls outside.
