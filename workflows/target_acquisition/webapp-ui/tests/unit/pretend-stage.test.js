...
describe("the pretend stage", () => {
  ...
  beforeEach(async () => { await backend.setXyz({ x: 4800, y: 3200, z: -412 }); });

  it("answers with micrometres per axis", async () => {
    const at = await backend.getXyz();
    expect(at.x.unit).toBe("um");
    expect(um(at)).toEqual({ x: 4800, y: 3200, z: -412 });
  });

  it("stays where it was driven", async () => {
    await backend.setXyz({ x: 61_000, y: 42_000, z: -380 });
    expect(um(await backend.getXyz())).toEqual({ x: 61_000, y: 42_000, z: -380 });
  });

  it("answers the drive with where it ended up", async () => {
    const at = await backend.setXyz({ x: 12_345, y: 6_789, z: -400 });
  ...
  it("stops at the ends of its travel rather than driving through them", async () => {
    ...
    expect(um(await backend.setXyz({ x: 999_999, y: -999_999, z: -412 })))
  ...
  it("leaves an axis alone when it is not asked about", async () => {
    ...
    await backend.setXyz({ x: 20_000, y: 30_000, z: -390 });
    expect(um(await backend.setXyz({ x: 25_000 })))
