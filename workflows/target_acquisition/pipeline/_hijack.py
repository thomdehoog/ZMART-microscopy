...
def _read_native_autosave_system_type(metadata_dir: Path) -> str | None:
    """Read SystemTypeName from copied native AutoSave vendor metadata.

    Native AutoSave canonical companion XML is generated by our writer and
    does not carry Leica's ``OriginalMetadata`` block. The source XLIF copied
    under ``metadata/vendor/lasx_native_autosave`` does carry Leica's
    ``SystemTypeName`` attribute. Accept it only when the vendor folder yields
    exactly one distinct non-empty value; missing, unreadable, or conflicting
    metadata fails closed as ``None``.
    """
    vendor_dir = metadata_dir / "vendor" / _NATIVE_AUTOSAVE_VENDOR
    values: set[str] = set()
...
def hijack_frame(
    ...
    *,
    ...
    # race.
    metadata_dir = layout.metadata_dir(kind)
    xml_path = metadata_dir / build_xml_name(result.naming)
    system_type = _read_system_type(xml_path)
    system_type_source = "companion XML"
    if system_type is None:
        system_type = _read_native_autosave_system_type(metadata_dir)
        system_type_source = "native AutoSave vendor metadata"
    if system_type != "SIMULATOR":
        raise NonSimulatorFrameError(
            f"refusing to overwrite {result.image_path.name}: "
            f"{system_type_source} SystemTypeName is {system_type!r}, not "
