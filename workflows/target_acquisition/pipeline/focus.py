...
import numpy as np
from navigator_expert.commands.commands import acquire as drv_acquire
...
class FocusMap:
    ...
    def plot(self, ctx: Context) -> None:
        ...
        ax = fig.add_axes([_FIELD_LEFT, field_bottom, _FIELD_WIDTH, _FIELD_HEIGHT * s])
        cbar_bar_in = 0.20
        cbar_below_in = 0.55
        ...
        cax = fig.add_axes([_FIELD_LEFT, cax_bottom, _FIELD_WIDTH, cax_height])
        ...
        cross = rc.max_tile_size_um * 0.25 if rc.max_tile_size_um else span * 0.01
        circle_r = cross * 0.6
        focus_color = "#e05555"
        ...
        ax.plot(
            [], [], "+", color=focus_color, markersize=10, markeredgewidth=1.5, label="Focus points"
        )
        if lim:
        ...
        sm.set_array([])
        plt.colorbar(sm, cax=cax, label="z-wide (um)", orientation="horizontal")
...
def build_focus_map(ctx: Context) -> FocusMap:
    ...
    if save_result is None:
        raise RuntimeError("drv.save_experiment failed. Cannot read focus positions.")
    ...
    focus_positions = af_data.get("focus_points", []) or af_data.get("autofocus_points", [])
    ...
    try:
        ...
        for i, fp in enumerate(focus_positions):
            ...
            y_um = fp["y_um"]
            print(
                f"\n[{i + 1}/{len(focus_positions)}] x={x_um:.0f}  y={y_um:.0f}",
                end="",
                flush=True,
            )
...
def _print_focus_diagnostics(fm: FocusMap, ctx: Context) -> None:
    ...
    if ctx.scan_field is not None:
        ...
        if (
            min(tile_xs) < min(fx)
            or max(tile_xs) > max(fx)
            or min(tile_ys) < min(fy)
            or max(tile_ys) > max(fy)
        ):
            print(
                "  WARNING: some tiles are outside the focus marker "
                "bounding box -Z values are extrapolated"
            )
