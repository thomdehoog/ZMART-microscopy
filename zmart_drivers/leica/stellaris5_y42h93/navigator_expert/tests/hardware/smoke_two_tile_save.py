...
1. Resolve the active save exporter from ``config.profiles.ACQUISITION``.
...
# -- import bootstrap: zmart_drivers/vendor/leica so `navigator_expert` imports.
        ...
        try:
            ...
            if not _same(actual, target, attempt["tolerance"]):
                try:
                    ...
                    errors.append(f"{name}: readback {actual!r} != target {target!r}")
                continue
...
def _xy_target(client: Any, delta_um: float) -> tuple[dict, tuple[float, float]]:
    ...
    if not _inside(x1, y1):
        x1, y1 = x0 - delta_um, y0 - delta_um
    if not _inside(x1, y1):
        raise SmokeFailure(f"no safe +/-{delta_um} um XY target from {(x0, y0)} within {limits}")
    return start, (x1, y1)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--limits-config", type=Path, help="optional stage limits JSON; default driver config"
    )
    p.add_argument("--job-a", help="first job; default selected job")
    p.add_argument("--job-b", help="second job; default first alternate job")
    p.add_argument("--save-exporter", help="override profile save exporter for this smoke test")
    p.add_argument(
        "--output-root", type=Path, help="SMART output root; default is a temp directory"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        report["tiles"].append(_acquire_and_save(client, job_a, output_root, hash6, 0, exporter))
        ...
        report["tiles"].append(_acquire_and_save(client, job_b, output_root, hash6, 1, exporter))
