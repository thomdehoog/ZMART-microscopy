...
# -- import bootstrap: add zmart_drivers/vendor/leica so `navigator_expert`
...
def probe_job(client, job):
    ...
    print(f"acquire ok: started={acq.started_at:.3f} finished={acq.finished_at:.3f}")
    ...
    print(
        f"images     : {len(exported.image_files)}   "
    ...
    for img in sorted(exported.image_files):
        ...
        in_wait = companion is not None and companion.resolve() in waited_xml
        flag = "ok" if in_wait else "NOT-IN-WAIT-SET"
        print(f"  {img.name}")
        print(
            f"      C={d.get('C')} Z={d.get('Z')} T={d.get('T')} "
            f"J={d.get('J')} repeat={d.get('repeat')}  shape={_shape(img)}"
        )
        print(f"      companion XML: {companion.name if companion else None}  [{flag}]")
    ...
    if len(exported.image_files) == 1:
        ...
        if getattr(arr, "ndim", 0) >= 3:
            print(
                "  -> ONE multi-page TIFF (Z/C live in pages). "
        ...
        print(
            f"  -> {len(exported.image_files)} separate files"
