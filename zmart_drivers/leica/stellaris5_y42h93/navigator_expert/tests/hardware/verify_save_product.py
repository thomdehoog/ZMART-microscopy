...
def _connect():
    from navigator_expert.connection.lasx_runtime import load_lasx_api_runtime
...
def main(argv) -> None:
    ...
    print("\nChecks:")
    check("summary source files resolved", not missing_sources, ", ".join(missing_sources[:3]))
    check(
        "one data/ file per source plane",
        len(out_imgs) == len(src_czt),
        f"{len(out_imgs)} vs {len(src_czt)}",
    )
    check("output (c,z,t) == source (C,Z,T)", out_czt == src_czt)
    check("one XML per T", len(out_xml) == len(src_t), f"{len(out_xml)} vs {len(src_t)}")
    check("Leica source export bytes UNCHANGED (repair hit the copy)", source_unchanged)
    check(
        "manifest image_paths complete",
        isinstance(img_paths, dict) and len(img_paths) == len(src_czt),
        f"{len(img_paths) if isinstance(img_paths, dict) else img_paths}",
    )
    check(
        "manifest xml_paths complete", isinstance(xml_paths, dict) and len(xml_paths) == len(src_t)
    )
    check("save() exposes no loaded pixel array (manifest only)", not hasattr(saved, "image"))
