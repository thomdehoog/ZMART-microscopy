...
class MockLasxClient:
    ...
    def __init__(self, latency=0.001):
        ...
        self.PyApiStatus = _MockApiObject("Status", lambda m: None, latency)
        ...
        self.PyApiCommand = _MockApiObject("Command", self._handle_command_dispatch, latency)
        ...
        self.PyApiGetJobSettingsByName = _MockApiObject("GetJobSettings", lambda m: None, latency)
        ...
        self.PyApiGetJobsInformation = _MockApiObject("GetJobs", self._handle_get_jobs, latency)
        ...
        # Action API objects
        self.PyApiSelectJobByName = _MockApiObject("SelectJob", self._handle_select_job, latency)
        self.PyApiStartScan = _MockApiObject("StartScan", self._handle_start_scan, latency)
        self.PyApiAcquireJob = _MockApiObject("AcquireJob", self._handle_acquire_job, latency)

        # Move commands — models need enum-like properties
        self.PyApiMoveHardwareXY = _MockApiObject("MoveStage", self._handle_move_stage, latency)
    ...
    def _handle_command_dispatch(self, model):
        ...
        if cmd == "GetXY":
            ...
            self._handle_get_job_settings(self.PyApiGetJobSettingsByName.Model)
        elif cmd == "GetConfocalHardwareInfo":
            self._handle_get_hardware_info(self.PyApiGetConfocalHardwareInfo.Model)
    ...
    def _make_set_handler(self, cmd_name):
        ...
        def handler(model):
            ...
            if time.monotonic() < self._scan_busy_until:
                self._echo.set_error("The parameter cannot be set while the block is being scanned")
                return

            job_name = getattr(model, "JobName", "")
            if job_name not in self._jobs:
                self._echo.set_error(
                    f"CamCommand{cmd_name.replace('PyApi', '')} invalid block identifier"
                )
                return
    ...
    def _set_zoom(self, model, job):
        ...
        if v is None or v < lo or v > hi:
            self._echo.set_error(f"The zoom parameter ({v}) is out of range. Valid: [{lo}, {hi}]")
            return
        job["zoom"]["current"] = v
    ...
    def _set_scan_speed(self, model, job):
        ...
        if v is None or v < lo or v > hi:
            self._echo.set_error(
                f"The scan speed parameter ({v}) is out of range. Valid: [{lo}, {hi}]"
            )
            return
    ...
    def _set_rotation(self, model, job):
        ...
        if v is None or v < lo or v > hi:
            self._echo.set_error(f"The rotation parameter ({v}) is out of range.")
            return
    ...
    def _set_z_stack_definition(self, model, job):
        if "stack" not in job or job["stack"] is None:
            job["stack"] = {"begin": None, "end": None, "stepSize": None, "size": None}
    ...
    def _set_frame_accumulation(self, model, job):
        ...
        if v is None or v < lo or v > hi:
            self._echo.set_error(
                f"The frame accumulation parameter ({v}) is invalid. Valid: [{lo}, {hi}]"
            )
            return
    ...
    def _set_frame_average(self, model, job):
        ...
        if v is None or v < lo or v > hi:
            self._echo.set_error(
                f"The frame average parameter ({v}) is invalid. Valid: [{lo}, {hi}]"
            )
            return
        si = getattr(model, "SettingIndex", 0)
        if si < len(job["activeSettings"]):
    ...
    def _set_line_accumulation(self, model, job):
        ...
        if v is None or v < lo or v > hi:
            self._echo.set_error(f"The line accumulation parameter ({v}) is invalid.")
            return
        si = getattr(model, "SettingIndex", 0)
        if si < len(job["activeSettings"]):
    ...
    def _set_line_average(self, model, job):
        ...
        if v is None or v < lo or v > hi:
            self._echo.set_error(f"The line average parameter ({v}) is invalid.")
            return
        si = getattr(model, "SettingIndex", 0)
        if si < len(job["activeSettings"]):
    ...
    def _set_pinhole(self, model, job):
        ...
        if v is not None and (v < lo or v > hi):
            ...
            self._echo.set_warning(f"pinhole adjusted to nearest valid value: {clamped}")
            if si < len(job["activeSettings"]):
    ...
    def _set_detector_gain(self, model, job):
        ...
        if si < len(job["activeSettings"]):
            ...
            if det is None:
                ...
                return
            if v is None or v < lo or v > hi:
                self._echo.set_error(f"The detector gain ({v}) is out of range.")
                return
            det["gain"]["value"] = v
    ...
    def _set_laser_intensity(self, model, job):
        ...
        if si < len(job["activeSettings"]):
            ...
            las = next((laser for laser in lasers if laser["beamRoute"] == br), None)
            if las is None:
                self._echo.set_error(f"Invalid light source: {br}")
                return
            if v is None or v < lo or v > hi:
                self._echo.set_error(f"The laser intensity ({v}) is out of range.")
                return
    ...
    def _handle_move_z(self, model, job):
        ...
        # so a confirmation that polls the settings on that drive cannot pass
        # here either — the reader/confirmation have to go through the saved
        # experiment, as on the instrument.
        stack = job.get("stack") or {}
        frozen = stack.get("mode")
