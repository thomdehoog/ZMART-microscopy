...
def lrp_set_scan_direction(lrp_path, bidirectional, job_name):
    ...
    count = 0
    count += _set_job_attr(
        lrp_path, "ScanDirectionX", str(mode), job_name, "lrp_set_scan_direction"
    )
    count += _set_job_attr(lrp_path, "ScanDirectionXName", name, job_name, "lrp_set_scan_direction")
    return count
...
def lrp_set_resonant_scanner(lrp_path, enable, job_name):
    ...
    count = 0
    count += _set_job_attr(lrp_path, "IsResonantScanner", val, job_name, "lrp_set_resonant_scanner")
    count += _set_job_attr(
        lrp_path, "IsResonantConfocalScanner", val, job_name, "lrp_set_resonant_scanner"
    )
    return count
...
def lrp_set_bit_depth(lrp_path, value, job_name):
    ...
    if value not in (8, 12, 16):
        log.error("lrp_set_bit_depth: invalid value %r (expected 8, 12, or 16)", value)
        return 0
