...
def strip_template(client, *, save_timeout=120):
    ...
    r = save_experiment(
        client, TEMPLATE_XML, templates_dir, timeout=save_timeout, confirm_path=rgn_path
    )
    if r is None:
    ...
    if "<ScanFieldData" in stripped_xml.read_text(encoding="utf-8"):
        log.warning("Strip incomplete — stripped XML still contains ScanFieldData elements")
    ...
    r = save_experiment(client, STRIPPED_XML, templates_dir, confirm_path=stripped_rgn)
    if r is None:
...
def strip_template_in_place(client, *, save_timeout=120):
    ...
    if sf > 0 or si > 0 or fp > 0:
        log.error(
            "Canonical template still has objects after strip: %d fields, %d items, %d focus",
            sf,
            si,
            fp,
        )
...
def restore_template(client):
    ...
    log.info(
        "Restoring template (expecting %d fields, %d items, %d focus)...",
        orig_fields,
        orig_items,
        orig_focus,
    )
    ...
    for attempt, save_timeout in enumerate(_RESTORE_SAVE_TIMEOUTS, 1):
        log.info(
            "Restore attempt %d/%d: loading original template...",
            attempt,
        ...
        if r is None:
            log.warning(
                "Confirm-save timed out on attempt %d — "
        ...
        log.warning(
            "Object count mismatch (expected >=%d/%d, got %d/%d) — restoring backup",
            orig_fields,
            orig_items,
            fields,
            items,
    ...
    for f in (stripped_xml, stripped_rgn, stripped_lrp, bak_xml, bak_rgn, bak_lrp):
        if f.is_file():
            f.unlink()

    log.info(
        "Restore complete in %.1fs (%d attempt%s): %d fields, %d items, %d focus",
        total_t,
        attempt,
        "s" if attempt > 1 else "",
        fields,
