...
> RDK family, and what it means for a `zmart_drivers/evident/` driver that sits beside
> `zmart_drivers/leica/stellaris5_y42h93/navigator_expert/`, `zmart_drivers/nikon/`, and `zmart_drivers/zeiss/zenapi/`.
>
...
- This is **architecturally identical to Leica LAS X CAM and Nikon NkSocket** — an external Python
  ...
  a **sibling `zmart_drivers/evident/...`** on the same neutral waist (connection + command vocabulary +
...
```
zmart_drivers/evident/fv4000/rdk/         (eventual path; vendor/machine/api)
...
The Nikon spike (`zmart_drivers/nikon/spike/`) is a text-over-TCP client with `?query`→reply framing. The
...
4. **Grow into `zmart_drivers/evident/fv4000/rdk/`** — copy the Leica skeleton, implement connection +
