...
> **Implementation amendment, 2026-08-09:** raw and seamless views are now
> strictly metadata-and-route-only at every advertised pyramid level. Canonical
> positions are sharded at every level, and the optimizer accepts a profile only
> when the complete frame, overlap and step are made of whole inner chunks at
> every power-of-two level. Earlier text below that permits physical outer-edge
> strips, copied coarse view levels, or a rechunking fallback is superseded by
...
- Every advertised raw and seamless view level points at existing position
  chunks. A profile that cannot do this is refused before acquisition.
...
3. **The seamless view is OME-Zarr 0.5 metadata with ZMART-specific pixel
   routing.** Every requested chunk is the already-encoded inner chunk in a
   canonical position. The linked chunk/range map is not part of standard
   OME-Zarr, so a generic reader can inspect the image description but needs the
   ZMART adapter/gateway to obtain pixels. Canonical positions remain the
   independently portable images.
...
- the complete, validated virtual-link/resolver state and both metadata-only
  view descriptions required by the published view contract; and
- validation that expected canonical chunks and both virtual routes can be opened.
...
- every raw and seamless virtual route at every advertised level; and
...
This also prevents a multi-channel route from becoming visible before every
channel in its canonical publication unit exists.

This selects **strict publication**. A position or timepoint is not temporarily
exposed while pyramids, links, ownership metadata, or virtual-view metadata are
still being completed in the background.
...
position/timepoint data and position pyramids complete
          |
          v
virtual links and both raw/seamless view descriptions complete
          |
          v
...
installed before the metadata-only views are refreshed. That makes the gateway
temporarily withhold the affected position while routes point at the candidate.
The manifest commit then reveals the new generation atomically. If any step
fails, both view descriptions are regenerated from committed units and the old
map is restored last. Readers therefore see the previous committed bytes or a
temporary refusal, never uncommitted replacement pixels.
...
- readiness of the advertised position pyramids and complete raw/seamless route sets;
- timestamp; and
...
The profile is agreed and sealed before the first unit of an acquisition instance is committed. Frame, overlap, inner chunks, shards, codecs, pyramid scheme, and visual/analysis ownership policies do not mutate underneath published data. A change to any incompatible profile field starts a new acquisition instance/profile; it never silently reinterprets earlier timepoints.
...
overlap:
  ...
  permitted_fraction: [0.10, 0.20]
  preferred_target: 0.125
```

Ten to twenty per cent is the ordinary default. Exact bounds belong to the
instrument/acquisition type and may deliberately be moved—for example to permit
9%—before its profile is sealed. The optimizer works in integer pixels and does
not require a round percentage; 12.5% is an ordinary valid result. It never
silently widens the configured band.

The profile builder enumerates candidate inner chunks and integer overlaps, then selects a compatible combination:

1. Enumerate integer overlaps inside the configured band that survive every exact
   power-of-two pyramid level.
2. At each level and on each tiled axis, choose a useful chunk that divides the
   level frame, overlap, and step; equivalently it divides their greatest common
   divisor.
3. Prefer the configured overlap target and a bounded logical request grid, while
   retaining non-dominated alternatives for an operator to inspect.
4. Choose every shard dimension as a whole multiple of that level's inner chunk
   and near the acquisition-type byte target.

If no candidate satisfies the hard alignment requirements, planning stops and
offers explicit remedies: change an adjustable scan format, use fewer pyramid
levels, permit a smaller chunk, or deliberately change the overlap band. The
strict acquisition views do not pad, trim, rechunk, or copy a boundary silently.

Where ZMART controls acquisition, the selected profile is sent to the microscope workflow and then stored with the run. For imported or externally controlled acquisitions, the same logic operates as a validator: compatible positions take the direct-link path, while incompatible or uncertain positions remain portable canonical images but are refused from this strict virtual acquisition view.
...
- the outermost row and column retain their complete camera edge through the same
  canonical-chunk routing.
...
For a 2304 × 2304 frame under the current 10–20% defaults, the optimizer chooses
288 pixels (12.5%) of overlap. Its chunks are 288, 144, 72, and 36 pixels at the
four exact power-of-two levels, leaving an 8 × 8 logical request grid throughout.
The complete examples and trade-offs are recorded in
[`zero-copy-acquisition-optimizer.md`](zero-copy-acquisition-optimizer.md).

One-sided ownership does **not** reduce the amount of unique specimen area in the
final seamless image. Its benefit is simpler ownership and a less restrictive
alignment rule. Its visual tradeoff is that the seam lies at one acquisition
edge rather than halfway through the overlap. The strict acquisition-view
implementation selects that trade-off; analysis ownership may still use a
midpoint or model-aware core because it does not have to align to chunks.
...
At a directly linked pyramid level `l`, let `d_l` be its downsampling relative
to level 0 and `C_l` its inner chunk in level-`l` pixels. For frame `F`, overlap
`O`, and step `S = F - O`, strict zero-copy requires:
...
The optimizer performs this independently on `y` and `x`, and chunks may change
between levels. Sharding does not relax the rule. Every advertised level must
satisfy it; there is no physical coarse-level fallback.

## Global raw and seamless coarse levels

There are no view-owned global coarse pixels in the selected implementation.
Canonical positions already hold every exact power-of-two pyramid level. Raw and
seamless views route those levels differently, so their maps differ while their
pixel bytes remain canonical. A commit validates both complete route sets before
advancing the visible revision.
...
3. **Views never own shards or chunks.** They contain array metadata and routes
   only; a legacy `views/**/c` payload tree is removed during refresh and causes
   readiness inspection to fail if it reappears.
...
- At every canonical pyramid level, span the complete position in `y` and `x`.
- Choose a bounded z-slab from that level's plane size and the acquisition-type
  byte target (currently 128–512 MiB), capped by the position depth.

For a 2304 × 2304 uint16 plane, one uncompressed plane is about 10.1 MiB.
The planner derives its z-slab from that fact rather than imposing one z count
on all microscopes. Real Windows/SMB timing remains required qualification.
...
7. Raw and seamless views contain no physical chunk payload and refresh by
   changing only metadata/routes.
...
12. A position with an incompatible acquisition storage profile is refused; it
   is never byte-linked under false metadata or silently copied into a view.
...
23. A timepoint remains invisible until all advertised position pyramids,
   virtual links/resolver state, ownership metadata, and both metadata-only view
   descriptions validate successfully.
