...
| Adapter `set_state`/`set_procedure` | ungated at the adapter, but every effect runs through gated wrappers (`select_job`, `move_xy` via `correct_backlash`, `acquire`) — verified refusals surface as RuntimeError in `test_mutating_ops_refuse_without_a_limits_handshake` |
