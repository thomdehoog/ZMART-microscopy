...
def _ms(x):
    return "%.0fms" % x if x is not None else "TIMEOUT"
...
def phase_readonly(client, results):
    ...
    if axy is None and lxy is not None:
        _row(results, "get_xy", True, f"API HANG ({alat:.0f}ms) -> log delivered "
             ...
             hang=True, latency=alat)
    else:
        ok = bool(axy and lxy and _eq(axy["x_um"], lxy["x_um"], 1.0) and _eq(axy["y_um"], lxy["y_um"], 1.0))
    ...
    _row(results, "get_jobs (names)", an == ln, f"api={an} log={ln}", latency=alat,
    ...
    for job in (ln or an):
        ...
        if af is None or lf is None:
            _row(results, f"settings[{job}]", False,
        ...
        diffs = [k for k in keys if not _ceq(af.get(k), lf.get(k),
...
def phase_changes(client, job, results):
    ...
    for name, cands, tol, setfn, readfn in specs:
        ...
        try:
            ...
            _row(results, f"change[{name}] -> {target}", both,
                 f"api={_ms(api_ms)} log={_ms(log_ms)} earlier={earlier}" + ("  API-HANG" if hang else ""),
                 hang=hang or api_ms is None, latency=api_ms or 0)
        finally:
            setfn(job, cur)
...
def phase_select(client, results):
    ...
    try:
        for n in names:
            ...
            _row(results, f"select[{n}]", both, f"api={_ms(api_ms)} log={_ms(log_ms)}{note}",
                 hang=api_ms is None, latency=api_ms or 0)
    finally:
        if original:
...
def main(argv=None):
    ...
    p.add_argument("--allow-job-switch", action="store_true",
