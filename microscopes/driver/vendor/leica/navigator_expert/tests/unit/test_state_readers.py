...
class TestStateReaders(unittest.TestCase):
    ...
    def test_hybrid_returns_log_when_api_hangs(self):
        ...
        with patch.object(router.api_reader, "get_xy", side_effect=slow_api), \
    ...
    def test_hybrid_prefers_fresh_log_over_faster_api(self):
        ...
        with patch.object(router.api_reader, "get_xy", return_value=api_value), \
             patch.object(router.log_reader, "parse_log", return_value=snapshot), \
    ...
    def test_hybrid_does_not_start_second_api_read_while_one_is_pending(self):
        ...
        with patch.object(router.api_reader, "get_xy", side_effect=blocked_api), \
             patch.object(router.log_reader, "parse_log", return_value=snapshot), \
             patch.object(router.log_reader, "get_xy", return_value=log_value), \
             patch.object(router.log_reader, "ages", return_value={"xy": 0.1}):
            first = threading.Thread(
                target=lambda: first_result.append(
                    state_readers.get_xy(client, diagnostics=True)
                ),
                daemon=True,
            )
            first.start()
            ...
            second = state_readers.get_xy(client, diagnostics=True)
            release_api.set()
    ...
    def test_confirm_select_job_pins_api_mode(self):
        ...
        with patch.object(confirmations._readers, "get_jobs", side_effect=fake_get_jobs), \
             patch("time.sleep"):
            result = confirmations.confirm_select_job(
                object(),
                job_name="Overview",
