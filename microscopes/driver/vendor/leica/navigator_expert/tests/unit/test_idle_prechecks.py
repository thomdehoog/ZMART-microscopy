...
IDLE_GUARDED = [
    ...
    "SELECT_JOB", "ACQUIRE", "ACQUIRE_SINGLE_IMAGE",
]
...
class TestIdlePrecheckPolicy(unittest.TestCase):
        ...
        for name in IDLE_GUARDED:
            ...
            with self.subTest(profile=name):
                self.assertIsNotNone(
                    profile.pre_check_fn,
                    f"{name} must see idle before firing")
                self.assertIsNone(
    ...
    def test_check_idle_waits_through_long_busy_phase(self):
        ...
        with patch.object(prechecks._readers, "get_scan_status",
                          side_effect=lambda c, **k: next(statuses)), \
             patch("time.sleep"):
