...
class TestDualLegRace(unittest.TestCase):
    def test_dual_leg_requires_budget(self):
        with self.assertRaises(ValueError):
            confirmations.race_confirmations(
                api_leg=lambda: _ok(), log_leg=lambda: _ok())
    ...
    def test_api_wins_and_log_disagreement_is_reported(self):
        ...
        self.assertTrue(result["success"])
        warnings = [
            entry["msg"] for entry in result["logs"]
            if entry["level"] == "warning"
        ]
        self.assertTrue(
            any("log leg had not confirmed" in m for m in warnings))
