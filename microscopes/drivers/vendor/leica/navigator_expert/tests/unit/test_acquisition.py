...
def fake_lasx_export(tmp_path: Path) -> dict:
    ...
    for z in range(2):
        for c in range(2):
            ...
            image_name = f"image--L0000--J08--E00--X00--Y00--T0000--Z{z:02d}--C{c:02d}.ome.tif"
    ...
    xml_path.write_bytes(
        b'<?xml version="1.0"?><OME><Image><Pixels SizeC="2" SizeZ="2" SizeT="1"/></Image></OME>'
    )
    now = time.time()
...
def patched_export(fake_lasx_export):
    ...
    with (
        patch.object(
            exporter._readers,
            "get_lasx_settings",
            return_value={"export": {"media_path": str(fake_lasx_export["media_path"])}},
        ),
        patch.object(
            exporter._files,
            "read_relative_path",
...
class TestSave:
    ...
    def test_save_generates_canonical_ome_and_preserves_source(
        ...
        image_path = source_dir / "image--L0000--J08--E00--X00--Y00--T0000--Z00--C00.ome.tif"
...
class TestHelpers:
    ...
    def test_find_companion_xml_with_repeat_suffix(self, tmp_path):
        ...
        metadata_dir.mkdir()
        image_path = (
            experiment_dir / "image--L0000--J08--E00--X00--Y00--T0000--Z00--C00--001.ome.tif"
        )
