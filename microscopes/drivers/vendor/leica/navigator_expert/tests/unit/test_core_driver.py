...
class TestModuleStructure(unittest.TestCase):
    def test_version(self):
        self.assertEqual(drv.__version__, "6.0.0")

    def test_no_api_set(self):
        self.assertFalse(hasattr(drv, "_api_set"))

    def test_readback_exists(self):
        self.assertTrue(callable(getattr(drv, "_readback", None)))

    def test_confirm_and_fire_exists(self):
        self.assertTrue(callable(getattr(drv, "confirm_and_fire", None)))

    def test_check_idle_exists(self):
        self.assertTrue(callable(getattr(drv, "check_idle", None)))
    ...
    def test_confirm_functions_in_confirm_module(self):
        for name in [
            ...
            with self.subTest(name=name):
                self.assertTrue(
                    callable(getattr(confirmations, name, None)), f"{name} missing in confirmations"
                )
...
class TestAcquisitionProtocol(unittest.TestCase):
    ...
    def test_full_protocol_completes(self):
        with (
            patch.object(drv, "select_job", side_effect=_mock_select),
    ...
    def test_job_switches_only_when_needed(self):
        ...
        with (
            patch.object(drv, "select_job", side_effect=mock_sel),
            patch.object(drv, "move_xy", side_effect=_mock_move),
            patch.object(drv, "acquire", side_effect=_mock_acq),
            patch.object(commands, "confirm_and_fire", side_effect=_mock_fire_ok),
        ):
    ...
    def test_per_position_speed_values(self):
        ...
        def mock_fire(client, api_obj, desc, **kw):
            ...
            if kw.get("setup_fn"):
                kw["setup_fn"](model)
            if "ScanSpeed" in desc:
                speeds.append(model.ScanSpeed)
            return _make_v6_result(msg=desc)
    ...
    def test_pinhole_only_set_when_specified(self):
        ...
        def mock_fire(client, api_obj, desc, **kw):
            ...
            if kw.get("setup_fn"):
                kw["setup_fn"](model)
            if "PinholeAiry" in desc:
                pinholes.append(model.PinholeAiry)
            return _make_v6_result(msg=desc)
    ...
    def test_laser_only_set_when_specified(self):
        ...
        def mock_fire(client, api_obj, desc, **kw):
            ...
            if kw.get("setup_fn"):
                kw["setup_fn"](model)
            if "Laser" in desc and "line" not in desc.lower():
                lasers.append(model.IntensityValue)
            return _make_v6_result(msg=desc)
    ...
    def test_total_api_call_counts(self):
        ...
        def mock_a(client, jn, **kw):
            counts["acquire"] += 1
            return _mock_acq(client, jn)

        def mock_fire(client, api_obj, desc, **kw):
            if "Zoom" in desc:
                counts["set_zoom"] += 1
            elif "ScanSpeed" in desc:
                counts["set_speed"] += 1
            elif "PinholeAiry" in desc:
                counts["set_pinhole"] += 1
            elif "Laser" in desc:
                counts["set_laser"] += 1
            return _make_v6_result(msg=desc)
...
class TestCommandReaderSafety(unittest.TestCase):
    def test_move_galvo_to_pixel_pins_parameter_reads_to_api(self):
        ...
        with (
            ...
            patch(
                "navigator_expert.runtime.utils.parse_tile_geometry",
