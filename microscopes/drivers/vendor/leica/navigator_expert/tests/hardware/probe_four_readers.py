...
def _read_passive(client, datum: str, mode: str, job_name: str | None) -> dict[str, Any]:
    if datum == "selected_job":
        return _timed_read(
            lambda: state_readers.get_selected_job(client, mode=mode, diagnostics=True)
        )
    if datum == "xy":
        return _timed_read(lambda: state_readers.get_xy(client, mode=mode, diagnostics=True))
    if datum == "jobs":
        return _timed_read(lambda: state_readers.get_jobs(client, mode=mode, diagnostics=True))
    if datum == "scan_status":
        return _timed_read(
            lambda: state_readers.get_scan_status(client, mode=mode, diagnostics=True)
        )
    if datum == "hardware_info":
        return _timed_read(
            lambda: state_readers.get_hardware_info(client, mode=mode, diagnostics=True)
        )
    if datum == "job_settings":
        ...
        return _timed_read(
            lambda: state_readers.get_job_settings(client, job_name, mode=mode, diagnostics=True)
        )
