...
class PositionKey:
    ...
    def label(self) -> str:
        return (
            f"{self.acquisition_type}:k{self.k}:m{self.m}:g{self.g}:p{self.p}:t{self.t}:v{self.v}"
        )
...
def _load_output(label: str, root: Path) -> SavedOutput:
    ...
    for i, record in enumerate(acqs):
        ...
        if existing_xml is not None and existing_xml != xml_path:
            raise SetupError(f"{label}: position {key.position.label()} maps to multiple XMLs")
...
def _record_naming(record: dict[str, Any], image_path: Path) -> dict[str, Any]:
    ...
    if parsed is None:
        raise SetupError(f"record lacks naming and image filename is not canonical: {image_path}")
    return {
...
def _compare_outputs(
    ...
    *,
    ...
    report["semantic_comparison"]["acquisition_type_normalized"] = not include_acquisition_type
    ...
    if nav_keys != native_keys:
        _add(
            ...
            "plane grids differ",
            details={
                "navigator_only": [_compare_key_label(k) for k in sorted(nav_keys - native_keys)],
                "native_only": [_compare_key_label(k) for k in sorted(native_keys - nav_keys)],
            },
        )
    if set(nav_xml) != set(native_xml):
        _add(
            ...
            details={
                "navigator_only": [
                    _compare_key_label(k) for k in sorted(set(nav_xml) - set(native_xml))
                ],
                "native_only": [
                    _compare_key_label(k) for k in sorted(set(native_xml) - set(nav_xml))
                ],
            },
        )

    xml_meta_nav = {key: _ome_metadata_from_file(path) for key, path in nav_xml.items()}
    xml_meta_native = {key: _ome_metadata_from_file(path) for key, path in native_xml.items()}
    ...
    image_info_nav = {key: _image_info(record.image_path) for key, record in nav_records.items()}
    image_info_native = {
        key: _image_info(record.image_path) for key, record in native_records.items()
    }
...
def _include_acquisition_type_in_compare(
    ...
    return not (len(nav_types) == 1 and len(native_types) == 1 and nav_types != native_types)
...
def _records_by_compare_key(
    ...
    for key, record in output.records.items():
        ...
        if compare_key in records:
            raise SetupError(
                f"{output.label}: duplicate normalized plane key {_compare_key_label(compare_key)}"
            )
...
def _ome_metadata_from_file(path: Path) -> dict[str, Any]:
    ...
    if pixels is not None:
        ...
        channels = [_clean_attrs(ch.attrib) for ch in _children_local(pixels, "Channel")]
    return {
        "path": str(path),
        "schema_version": _schema_version(root),
        "sizes": sizes,
...
def _assess_fair_readiness(output: SavedOutput, report: dict[str, Any]) -> None:
    ...
    xml_meta = {pos: _ome_metadata_from_file(path) for pos, path in output.xml_by_position.items()}
    filename_refs = sorted(
        {ref for meta in xml_meta.values() for ref in meta.get("filename_refs", [])}
    )
    canonical_basenames = {p.name for p in image_paths}
    source_refs = [ref for ref in filename_refs if Path(ref).name not in canonical_basenames]
    if source_refs:
        _add(
            report,
            "warnings",
    ...
    channel_metadata_present = all(bool(meta.get("channels")) for meta in xml_meta.values())
    ...
    report["fair_readiness"][output.label] = {
        ...
        "reusable_provenance": {
            ...
            "materialization_step_recorded": any("materialization_step" in r.raw for r in records),
        },
        "checksums_sha256": {_rel_display(p, output.root): _sha256(p) for p in checksum_paths},
    }
...
def _clean_attrs(attrs: dict[str, Any]) -> dict[str, str]:
    return {_local_name(str(k)): str(v) for k, v in sorted(attrs.items())}
