...
def lrp_set_stack_calculation_mode(lrp_path, mode, job_name):
    ...
    if mode not in STACK_MODES:
        log.error("lrp_set_stack_calculation_mode: invalid mode %r (expected 0, 1, or 2)", mode)
        return 0
    ...
    if m and m.group(1) != str(mode):
        new_element = new_element.replace(m.group(0), f'StackCalculationMode="{mode}"')
        count += 1

    target_name = STACK_MODES[mode]
    m = re.search(r'StackCalculationModeName="([^"]*)"', new_element)
    if m and m.group(1) != target_name:
        new_element = new_element.replace(m.group(0), f'StackCalculationModeName="{target_name}"')
        count += 1
    ...
    log.info(
        "lrp_set_stack_calculation_mode: job='%s', mode=%d (%s), %d attributes changed",
        job_name,
        mode,
        STACK_MODES[mode],
        count,
