...
from ..runtime.utils import RECEIPT_TIMEOUT, _make_log_entry, _make_timing
...
def _confirmation_detail(result):
    ...
    if not isinstance(result, dict):
        return ""
    details = {key: value for key, value in result.items() if key not in {"success", "logs"}}
    if not details:
        return ""
...
def confirm_and_fire(
    ...
    *,
    ...
    if not fb["success"]:
        dialog_detail = _append_dialog_warning(all_logs, command_started_at, description)
        return {
            "success": False,
            "confirmed": None,
