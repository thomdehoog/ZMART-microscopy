...
| `rdk_client.py` | the **`RdkClient`** — connect/login/command/query + placeholder device convenience methods. Seed of the eventual `drivers/evident/fv4000/rdk/connection`. |
| `test_rdk_roundtrip.py` | offline tests: pure protocol + full client↔mock-server round-trip. |

## Run

```powershell
python -m pytest -q drivers/evident/spike/test_rdk_roundtrip.py
```
...
4. **Grow into `drivers/evident/fv4000/rdk/`** — lift this client into a `connection` layer and add the
