...
Safety rules carried over from the routed readers:
...
def _log_reading(spec, parse_fn):
    try:
        ...
        age_s = None if observed_at is None else max(0.0, snapshot.now - observed_at)
        return router.Reading(
            value=value, source="log", observed_at=observed_at, age_s=age_s, error=None
        )
    except Exception as exc:
        return router.Reading(value=None, source="log", observed_at=None, age_s=None, error=exc)
...
def read_change_baseline(client, datum, *, parse_fn=None, api_timeout_s=None):
    ...
    profile = _profile()
    api_timeout_s = (
        profile.change_wait_baseline_api_timeout_s if api_timeout_s is None else api_timeout_s
    )
    parse_fn = log_reader.parse_log if parse_fn is None else parse_fn
    taken_at = time.time()

    api_queue = router._fire_api_read(lambda: spec.api_fn(client), router._client_api_key(client))
    ...
    log_reading, log_reason = _validated(_log_reading(spec, parse_fn), spec.key_fn)
...
def wait_for_change(
    ...
    *,
    ...
    timeout_s = profile.change_wait_timeout_s if timeout_s is None else timeout_s
    loop_interval_s = (
        profile.change_wait_loop_interval_s if loop_interval_s is None else loop_interval_s
    )
    api_retry_interval_s = (
        profile.change_wait_api_retry_interval_s
        if api_retry_interval_s is None
        else api_retry_interval_s
    )
    ...
    if baseline.datum != datum:
        raise ValueError(f"baseline datum {baseline.datum!r} does not match {datum!r}")
    ...
    while True:
        ...
        if api_queue is None and now >= api_due_at:
            api_queue = router._fire_api_read(lambda: spec.api_fn(client), api_key)
    ...
    if target_key is not None:
        ...
        if spec.numeric:
            if tolerance is not None:
                within_tolerance = target_delta is not None and target_delta <= tolerance
