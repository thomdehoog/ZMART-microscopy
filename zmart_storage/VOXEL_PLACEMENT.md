...
begins at 900.2500 µm. Each coarse voxel covers precisely the fine voxels it
stands for — one voxel of level 3 spans exactly the eight voxels of level 0 it was
made from — so the copies are perfectly nested and the picture does not move as a
viewer switches between them while zooming.
...
## 3a. Which fine voxel a coarse one *is*, and who that catches out

Added 6 August 2026, and, like the rest of this document, **nothing was changed as
a result**. The question in section 3 was whether the levels line up with each
other. This one is whether the *value* in a coarse voxel sits where the file says
its voxel sits, and it is a different question with a different answer.

**What is on disk, read back rather than remembered.** A run declared with 2 µm
planes and 0.35 µm pixels, four levels deep, comes back as:

```
level 0  scale [1, 1, 2.0, 0.35, 0.35]
level 1  scale [1, 1, 2.0, 0.70, 0.70]     level 1 == level 0[:, ::2, ::2]     exactly
level 2  scale [1, 1, 2.0, 1.40, 1.40]     level 2 == level 0[:, ::4, ::4]     exactly
level 3  scale [1, 1, 2.0, 2.80, 2.80]     level 3 == level 0[:, ::8, ::8]     exactly
multiscale-wide  translation [0, 0, origin_z, origin_y, origin_x]
```

Two things to read out of that. **No level carries a translation of its own** —
there is one translation for the whole pyramid, which for a run at the origin is
`[0, 0, 0, 0, 0]`. And **the levels are built by decimation from index 0**: every
second voxel, kept, starting at the low edge, along y and x only. None of them is a
block average; the writer says so itself, in the `metadata` block beside the
`"type": "nearest"` it declares.

**The consequence.** A coarse voxel's *extent* is the block of `2^k × 2^k` fine
voxels it covers, and its *value* is the one fine voxel at the low corner of that
block. Those two things are not centred on the same place. A reader that assumes
each coarse voxel is the average of its block — that is, that the value belongs at
the block's middle — will therefore place it

    (2^k − 1) / 2 fine voxels too far along, in y and in x

which is 0.5 fine voxels at level 1, 1.5 at level 2, 3.5 at level 3. **Along y and
x only, today**, because z is not shrunk at all: every level keeps the same 2 µm
plane spacing, so along z there is nothing to be out by. If z downsampling is ever
added, the same arithmetic starts applying to z as well, and that is the moment to
reopen this.

**Why it is left as it is.** Decimation is not a shortcut here, it is what the
whole arrangement stands on: because no voxels are combined, a coarse voxel belongs
to exactly one position, which is what lets a view point at a position's own file
instead of writing a copy (`how_many_copies_a_position_can_keep` sets this out).
Averaging near a tile join would end that. And the alternative repair — a per-level
translation of `(2^k − 1)/2 × 0.35 µm` — would be describing our decimated pyramid
in the language of an averaged one, which is a second convention layered on top of
the corner-versus-centre one already in this document. One undeclared convention is
enough.

So the file states what is true and the writer declares how the levels were made.
A reader that ignores the declaration and assumes averaging is wrong about our
data in a way it would also be wrong about anybody else's nearest-neighbour
pyramid.

## 3b. If z downsampling is ever wanted, the lab already has a convention
...
The mesoSPIM transfer on the share
(`Z:/zmbstaff/10637/Raw_Data/mesoSPIM_transfer_20260626_1700`) writes this, read
off one of its tiles:

```
level    0      1      2      3       4       5
z     10.0   10.0   10.0   10.0    20.0    40.0   µm
xy     1.1    2.2    4.4    8.8    17.6    35.2   µm
```

**z is held while it is still the coarser axis, and joins in as soon as halving xy
again would take the pixel past it.** At level 4 the new xy would be 17.6 against
z's 10, so z doubles; at level 5 it would be 35.2 against 20, so z doubles again.
The effect is a pyramid that starts at about 9:1 and ends at about 1.1:1 — nearly
isotropic at the top — rather than one that keeps halving xy until the pixel is
three times coarser than the plane spacing, which is what leaving z alone all the
way up would give.

Note also that this transfer repeats **the same translation on every level**, just
as we do, and declares no `type` or `metadata` saying how its copies were made. So
it applies no per-level compensation of any kind either, and a reader cannot tell
from the file whether it averaged or decimated. That is worth knowing before
treating section 3a as something peculiar to us.

Applied to a run this writer produces — 2 µm planes and 0.325 µm pixels — xy would
pass z at level 3 (0.325 → 0.65 → 1.3 → **2.6**), so z would first halve there.
That is the schedule to implement if it is ever implemented, and it would make our
stores the same shape as the transfers we already read.

**And it is six changes, not one.** They were worked out on 6 August 2026 while
this was being turned down, and they are listed here because the first attempt at
proposing it named none of them:
...
3. the concurrency guard in `canvas.py:_pieces_this_tile_touches` and `_touches`;
4. level depths that round up rather than down;
5. a schedule that is a pure function of `(level, voxel_size_um)` and never of the
   array's shape;
6. `voxel_size_um` kept on `TileCanvases`, which today takes it only as an argument
   to `create` and never retains it.

There is also nothing guarding it: `test_the_linked_view_matches_the_canvas.py`
places every tile at z = 0, so it would need tiles at two or more z offsets, one of
them deliberately unaligned, before it said anything at all about that axis.
