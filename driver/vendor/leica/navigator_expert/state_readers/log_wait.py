...
def wait_for_selected_job_log(
    ...
    *,
    ...
    profile = _profile()
    timeout_s = (
        profile.selected_job_log_poll_timeout_s
...
def _cluster_diagnostics(snapshot: log_reader.Snapshot, *, max_age_s):
    ...
    all_names = {
        j.get("jobName")
        for (j, _ts) in snapshot.atl_by_block.values()
        if j.get("jobName")
    }
    block_ids_numeric = all(
        log_reader._block_id_int(block_id) is not None
        for block_id, _job, _ts in latest.values()
    )
    return {
