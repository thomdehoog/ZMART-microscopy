...
def _inside(x, y, lim):
    return (lim["x_min"] <= x <= lim["x_max"]
            and lim["y_min"] <= y <= lim["y_max"])


def main(argv=None):
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--delta-um", type=float, default=5000.0,
                   help="square leg length in micrometers (default 5000)")
    p.add_argument("--laps", type=int, default=2,
                   help="how many times to go around the square")
    p.add_argument("--pause-s", type=float, default=0.8,
                   help="pause at each waypoint so you can see it")
    p.add_argument("--readback", choices=["both", "api", "log"], default="both",
                   help="which reader(s) to read the position back with")
    p.add_argument("--log-poll-s", type=float, default=1.5,
    ...
    lim = dict(
        x_min=_l["x"][0], x_max=_l["x"][1],
        y_min=_l["y"][0], y_max=_l["y"][1],
        z_galvo_min=_l["z_galvo"][0], z_galvo_max=_l["z_galvo"][1],
        z_wide_min=_l["z_wide"][0], z_wide_max=_l["z_wide"][1],
    )
    ...
    print(f"stage limits = x[{lim['x_min']:.0f},{lim['x_max']:.0f}] "
    ...
    for name, tx, ty in legs:
        if not _inside(tx, ty, lim):
            raise SystemExit(
                f"target '{name}' ({tx:.0f},{ty:.0f}) outside stage limits - "
                "reduce --delta-um")
    ...
    def read_back(mode):
        ...
        while True:
            ...
            pos = _xy(v)
            done = (pos is not None or mode != "log"
                    or (time.perf_counter() - t0) >= args.log_poll_s)
            if done:
    ...
    try:
        ...
        print(f"restored -> {_xy(drv.get_xy(client, mode='api'))}  "
              f"(ok={r.get('success')})")
    ...
    for m in modes:
        ...
        if ms:
            print(f"  {m}: returned a value {hit}/{n}; "
