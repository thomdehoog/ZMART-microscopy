...
def _read_passive(client, datum: str, mode: str, job_name: str | None) -> dict[str, Any]:
    if datum == "selected_job":
        return _timed_read(
            lambda: state_readers.get_selected_job(
                client, mode=mode, diagnostics=True))
    if datum == "xy":
        return _timed_read(
            lambda: state_readers.get_xy(client, mode=mode, diagnostics=True))
    if datum == "jobs":
        return _timed_read(
            lambda: state_readers.get_jobs(client, mode=mode, diagnostics=True))
    if datum == "scan_status":
        return _timed_read(
            lambda: state_readers.get_scan_status(
                client, mode=mode, diagnostics=True))
    if datum == "hardware_info":
        return _timed_read(
            lambda: state_readers.get_hardware_info(
                client, mode=mode, diagnostics=True))
    if datum == "job_settings":
        ...
        return _timed_read(
            lambda: state_readers.get_job_settings(
                client, job_name, mode=mode, diagnostics=True))
...
def _read_all_passive(
    ...
    *,
    ...
    return {
        mode: _read_passive(client, datum, mode, job_name)
...
def _change_wait_record(result: Any, fired_at: float) -> dict[str, Any]:
    noticed_after_fire_s = (
        None if result.observed_at is None else result.observed_at - fired_at
    )
    return {
...
def _summarize_selected(reads: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return {
        mode: _selected_name(read.get("value") if read else None)
...
def phase_read_only(
    ...
    for datum in ("selected_job", "xy", "jobs", "scan_status", "hardware_info"):
        ...
        baseline = (
            _read_change_baseline(client, datum)
            if datum in {"selected_job", "xy"} else None
        )
        record = {
            "phase": "read_only",
            "datum": datum,
    ...
    pending = _timed_read(lambda: state_readers.get_pending_dialog(diagnostics=True))
...
def _current_selected_job(client) -> str | None:
    selected = state_readers.get_selected_job(client, mode="api")
...
def _job_sequence(
    ...
    *,
    ...
    jobs = state_readers.get_jobs(client, mode="api") or []
            ...
            for target in sequence:
                ...
                record = {
                    ...
                    "command": {
                        ...
                        "success": (
                            None if command is None else command.get("success")
                        ),
                        "confirmed": (
                            None if command is None else command.get("confirmed")
                        ),
                    },
...
def _api_xy(client) -> tuple[float, float] | None:
    value = state_readers.get_xy(client, mode="api")
    pair = _xy_pair(value)
...
def phase_xy_positions(
    ...
    *,
    ...
    try:
        ...
        print(
            f"Restoring original XY: ({original[0]:.1f}, {original[1]:.1f}) um"
        )
...
def _build_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for record in records:
        ...
        if phase == "read_only":
            ...
            ok = ok and all(
                read.get("status") not in {"error", "exception"}
                for read in passive.values()
            )
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument("--read-only", action="store_true",
                        help="skip job changes and XY moves")
    parser.add_argument("--yes", action="store_true",
                        help="allow reversible job changes and XY moves")
    ...
    parser.add_argument("--all-jobs", action="store_true",
                        help="select every API-visible job in each round")
    parser.add_argument("--job-rounds", type=int, default=2,
                        help="number of job-change rounds (default 2)")
    parser.add_argument("--positions", type=int, default=10,
                        help="number of XY targets (default 10)")
    parser.add_argument("--xy-step-um", type=float, default=100.0,
                        help="radius of XY position pattern around current XY")
    parser.add_argument("--xy-tolerance-um", type=float, default=20.0,
                        help="reported XY target tolerance")
    parser.add_argument("--output", default=None,
                        help="JSONL output path")
    parser.add_argument("--append", action="store_true",
                        help="append if --output already exists")
...
def main(argv=None) -> int:
    ...
    output = Path(args.output) if args.output else (
        HERE / f"probe_four_readers_{_stamp()}_results.jsonl"
    )
