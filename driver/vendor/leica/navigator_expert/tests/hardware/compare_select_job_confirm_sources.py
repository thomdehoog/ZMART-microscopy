        ...
        for run in range(1, args.runs + 1):
            ...
            if output.exists() and not args.append:
                raise FileExistsError(
                    f"{output} already exists. Use a new --stamp or --append."
                )
...
def _parse_outputs(run_records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "log_faster_count": sum(
            1 for diff in paired_diffs if diff["log_minus_api_s"] < 0
        ),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--runs", type=int, default=10,
                        help="number of validator runs per source")
    parser.add_argument("--yes", action="store_true",
                        help="confirm live reversible writes")
    ...
    parser.add_argument("--stamp", default=None,
                        help="output filename timestamp; default: now")
    parser.add_argument("--append", action="store_true",
                        help="append to existing per-run JSONL outputs")
    parser.add_argument("--prime-log-select-cluster", action="store_true",
                        help="prime log job clusters before log confirmation")
    parser.add_argument("--log-select-confirm-timeout-s", type=float,
                        default=None,
                        help="override log selected-job confirmation timeout")
    parser.add_argument("--log-select-cluster-max-age-s", type=float,
                        default=None,
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary_path = output_dir / (
        f"select_job_confirm_source_compare_{stamp}_summary.json"
    )
    ...
    any_record_failed = any(
        record.get("status") == "FAIL"
        for record in summary["nonpass_records"]
    )
