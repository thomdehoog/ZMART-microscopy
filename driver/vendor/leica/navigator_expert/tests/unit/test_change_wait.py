...
class TestWaitForChangeSelectedJob(ChangeWaitTestCase):
    def test_log_change_wins_and_conflict_with_stale_api_is_reported(self):
        ...
        with patch.object(change_wait.api_reader, "get_selected_job",
            ...
            baseline = change_wait.read_change_baseline(
                ...
                parse_fn=lambda: _snapshot(
                    time.time(), block="Overview", block_ts=baseline_ts))
        ...
        self.assertEqual(
            result.diagnostics["last_valid"]["api"]["key"], "Overview")
    ...
    def test_stale_log_line_cannot_signal_change(self):
        ...
        baseline = change_wait.ChangeBaseline(
            ...
            log=router.Reading(
                ...
                observed_at=stale_ts, age_s=60.0),
            diagnostics={},
        )
...
class TestWaitForChangeXY(ChangeWaitTestCase):
    def _baseline(self, x=100.0, y=200.0):
        return change_wait.ChangeBaseline(
            ...
            taken_at=time.time(),
            api=router.Reading(
                value={"x_um": x, "y_um": y}, source="api",
                observed_at=time.time(), age_s=0.0),
            log=None,
            diagnostics={},
        )
    ...
    def test_api_change_wins_when_log_is_silent(self):
        ...
        with patch.object(change_wait.api_reader, "get_xy",
             ...
             patch.object(change_wait.log_reader, "get_xy",
                          return_value=None), \
             patch.object(change_wait.log_reader, "ages",
                          return_value={"xy": None}):
            result = change_wait.wait_for_change(
                self.client, "xy", self._baseline(),
                target=(150.0, 200.0), tolerance=1.0,
                parse_fn=lambda: _snapshot(time.time()))
        self.assertTrue(result.success)
        self.assertEqual(result.source, "api")
    ...
    def test_tolerance_is_reported_not_enforced(self):
        ...
        with patch.object(change_wait.api_reader, "get_xy",
            ...
            result = change_wait.wait_for_change(
                self.client, "xy", self._baseline(),
                target=(500.0, 500.0), tolerance=1.0,
                parse_fn=lambda: _snapshot(time.time()))
    ...
    def test_api_without_baseline_cannot_confirm(self):
        ...
        self.assertFalse(result.success)
        self.assertEqual(
            result.diagnostics["last_reasons"]["api"], "no_baseline")
...
class TestHangImmunity(ChangeWaitTestCase):
    def test_hung_api_does_not_block_the_log_leg(self):
        ...
        baseline = change_wait.ChangeBaseline(
            ...
            log=router.Reading(
                value={"Name": "Overview"}, source="log",
                observed_at=baseline_ts, age_s=5.0),
            diagnostics={},
        )
        calls = {"n": 0}
