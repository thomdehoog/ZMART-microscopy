...
| Microscope | API | Driver | Status |
|---|---|---|---|
| mesoSPIM (open-source light-sheet) | mesoSPIM-control Remote Control (TCP; [PR #106](https://github.com/mesoSPIM/mesoSPIM-control/pull/106)) | [`zmart_drivers/mesospim/`](zmart_drivers/mesospim/README.md) | **Ready for PR #106** — the driver speaks mesoSPIM-control's new Remote Control server (named, validated calls; one change at a time; operations polled to completion); offline suite green (174) against a faithful mock of that server; the full round-trip **incl. `acquire`** was demo-validated on the two earlier transports. Live `-D` demo run on the new server and real-hardware validation pending the pull request |
| ZEISS (ZEN) | ZEN API (gRPC) | [`zmart_drivers/zeiss/zenapi/`](zmart_drivers/zeiss/zenapi/README.md) | **Minimum viable product, plugged into the controller** — adapter with origin, stage envelope, moves, objective state and experiment runs saved with their printed state; full offline suite green (64); not yet bench-validated (see [Risks](zmart_drivers/zeiss/zenapi/README.md#10-risks--bench-verify)) |
