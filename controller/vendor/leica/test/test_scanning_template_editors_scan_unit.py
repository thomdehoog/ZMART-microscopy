...
from lasx.scanning_template_editors_scan import (
    ...
    set_scan_direction, verify_scan_direction,
...
class TestSetPan:
    def test_changes_both_dims(self, lrp_file):
        count = set_pan(lrp_file, 0.25, -0.1, "HiRes")
    ...
    def test_same_values_no_change(self, lrp_file):
        count = set_pan(lrp_file, 0, 0, "HiRes")
        assert count == 0

    def test_missing_job(self, lrp_file):
        count = set_pan(lrp_file, 0.5, 0.5, "NoSuchJob")
        assert count == 0

    def test_roundtrip(self, lrp_file):
        for x, y in [(0.1, 0.2), (-0.5, 0.75), (0.0, 0.0)]:
            set_pan(lrp_file, x, y, "HiRes")
            assert verify_pan(lrp_file, x, y, "HiRes")


class TestVerifyPan:
    def test_correct_values(self, lrp_file):
        assert verify_pan(lrp_file, 0, 0, "HiRes") is True

    def test_wrong_x(self, lrp_file):
        assert verify_pan(lrp_file, 1.0, 0, "HiRes") is False

    def test_wrong_y(self, lrp_file):
        assert verify_pan(lrp_file, 0, 1.0, "HiRes") is False

    def test_missing_job(self, lrp_file):
        assert verify_pan(lrp_file, 0, 0, "NoSuchJob") is False

    def test_tolerance(self, lrp_file):
        set_pan(lrp_file, 0.1005, 0.2005, "HiRes")
        # Within default tolerance of 0.001
        assert verify_pan(lrp_file, 0.1005, 0.2005, "HiRes") is True
        assert verify_pan(lrp_file, 0.1006, 0.2006, "HiRes",
                          tolerance=0.001) is True
        # Outside tolerance
        assert verify_pan(lrp_file, 0.11, 0.2005, "HiRes",
...
class TestSetZoom:
    def test_changes_zoom(self, lrp_file):
        count = set_zoom(lrp_file, 4.0, "HiRes")
        assert count == 2

    def test_same_zoom_no_change(self, lrp_file):
        count = set_zoom(lrp_file, 2.0, "HiRes")
        assert count == 0

    def test_roundtrip(self, lrp_file):
        set_zoom(lrp_file, 3.5, "HiRes")
        assert verify_zoom(lrp_file, 3.5, "HiRes")
...
class TestSetScanSpeed:
    def test_changes_speed(self, lrp_file):
        count = set_scan_speed(lrp_file, 400, "HiRes")
        assert count == 2

    def test_same_speed_no_change(self, lrp_file):
        count = set_scan_speed(lrp_file, 200, "HiRes")
        assert count == 0

    def test_roundtrip(self, lrp_file):
        set_scan_speed(lrp_file, 100, "HiRes")
        assert verify_scan_speed(lrp_file, 100, "HiRes")
...
class TestSetScanDirection:
    def test_to_unidirectional(self, lrp_file):
        count = set_scan_direction(lrp_file, False, "HiRes")
        assert count == 4  # 2 settings x 2 attributes

    def test_same_direction_no_change(self, lrp_file):
        count = set_scan_direction(lrp_file, True, "HiRes")
        assert count == 0

    def test_roundtrip(self, lrp_file):
        for bidir in (True, False, True):
            set_scan_direction(lrp_file, bidir, "HiRes")
            assert verify_scan_direction(lrp_file, bidir, "HiRes")
...
class TestSetScanFieldRotation:
    def test_changes_rotation(self, lrp_file):
        count = set_scan_field_rotation(lrp_file, 45.0, "HiRes")
        assert count == 2

    def test_same_rotation_no_change(self, lrp_file):
        count = set_scan_field_rotation(lrp_file, 0, "HiRes")
        assert count == 0

    def test_roundtrip(self, lrp_file):
        set_scan_field_rotation(lrp_file, 90.0, "HiRes")
        assert verify_scan_field_rotation(lrp_file, 90.0, "HiRes")
