...
def _default_output_dir() -> Path:
    """Default output is config/objective_targeting/stage/<timestamp>/."""
...
def apply_stage_limits_from_config() -> None:
...
def acquire_one_frame(client: Any, job: str) -> tuple[np.ndarray, Path]:
    """Trigger one acquisition and return (image_array, .ome.tif path).

    Polls for the new file on disk, waits for it to be unlocked +
    size-stable, then loads it. 3-D OME-TIFFs collapse to 2-D using
    the first plane.
    """
...
def step_setup(args: argparse.Namespace) -> tuple[Any, dict[str, Any], Any, Path]:
    """Prepare the world: connect, validate, set limits, idle check.

    Returns (client, calibration_cfg, hardware_info, output_dir).
    """
...
def step_acquire_source(
    ...
    img, lasx_path = acquire_one_frame(client, args.job)
...
def step_switch_and_aim(
    client: Any, args: argparse.Namespace, hw: Any,
    target_xyz_um: tuple[float, float, float],
    bbox_um: tuple[float, float],
) -> tuple[int, float, float]:
    """Switch to the target objective, set z-wide + zoom + stage XY.

    Returns (zoom, target_base_fov_um, target_pixel_size_um).
    """
    ...
    # Z FIRST. The firmware moved z-wide on the switch; we override
    # with the absolute value the translator computed.
    ...
    return zoom, target_base_fov_um, float(target_pixel_size_um)
...
def step_save_outputs(
    ...
    summary: dict[str, Any] = {
        ...
        "calibration_config": str(drv.default_current_path()),
        "outputs": {
