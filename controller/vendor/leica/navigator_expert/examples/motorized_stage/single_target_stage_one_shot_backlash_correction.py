"""
Single-target objective switch using the motorized XY stage, with stage
backlash correction applied before each acquisition.
=====================================================================

Identical recipe to ``single_target_stage_one_shot.py``, with one extra
step: just before each acquisition, the stage performs a takeup move
that drives it to ``(x - overshoot, y - overshoot)`` and back to
``(x, y)``. This pins the slack-state of both leadscrews to the +X+Y
side, so the source coordinates and the target landing are measured in
...
Usage
-----
    python single_target_stage_one_shot_backlash_correction.py --job HiRes \\
...
log = logging.getLogger("single_target_stage_one_shot_backlash_correction")
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "Single-target objective switch using only the motorized XY "
            "stage, with backlash correction before each acquisition."
        )
    )
...
def _default_output_dir():
    ...
    return (Path(__file__).resolve().parents[3]
            / "config" / "cookbook" / "motorized_stage"
...
def main():
    ...
    summary = {
        ...
        "method": "single_target_stage_one_shot_backlash_correction",
