...
from navigator_expert.driver.experimental.lrp_edits.focus import (
    STACK_MODES,
...
class TestSetStackCalculationMode:
    def test_changes_mode(self, lrp_file):
        count = lrp_set_stack_calculation_mode(lrp_file, 0, "AF Job")
        assert count == 2
    ...
    def test_same_mode_no_change(self, lrp_file):
        count = lrp_set_stack_calculation_mode(lrp_file, 1, "AF Job")
        assert count == 0

    def test_invalid_mode(self, lrp_file):
        count = lrp_set_stack_calculation_mode(lrp_file, 99, "AF Job")
        assert count == 0

    def test_missing_job(self, lrp_file):
        count = lrp_set_stack_calculation_mode(lrp_file, 0, "NoSuchJob")
        assert count == 0

    def test_all_modes_roundtrip(self, lrp_file):
        for mode in (0, 1, 2):
            lrp_set_stack_calculation_mode(lrp_file, mode, "AF Job")
            assert lrp_verify_stack_calculation_mode(lrp_file, mode, "AF Job")
...
class TestVerifyStackCalculationMode:
    def test_correct_mode(self, lrp_file):
        assert lrp_verify_stack_calculation_mode(lrp_file, 1, "AF Job") is True

    def test_wrong_mode(self, lrp_file):
        assert lrp_verify_stack_calculation_mode(lrp_file, 0, "AF Job") is False

    def test_missing_job(self, lrp_file):
        assert lrp_verify_stack_calculation_mode(lrp_file, 1, "NoSuchJob") is False
