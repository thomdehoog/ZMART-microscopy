...
def infer_overlap_pct_from_geometry_counts(
    ...
    *,
    ...
    for step in range(0, 501):
        overlap = step / 10.0
        generated = _generate_from_geometries(
            geometries, tile_size_um, overlap)
        score = sum(
            abs(len(generated.get(geom_id, [])) - count)
            for geom_id, count in expected
        )
        if score == 0:
    ...
    if exact:
        ...
        integer_candidates = [
            value for value in exact if abs(value - round(value)) < 1e-9
        ]
...
def has_lasx_tile_count_tags(geometries: dict[str, dict[str, Any]]) -> bool:
    ...
    return any(
        _tag_tile_count(geom) is not None
...
def _grid_count(dimension: float, tile_size: float, step: float,
                tol: float = 0.05) -> int:
        ...
        for ix in range(nx):
            for iy in range(ny):
                ...
                if _tile_overlaps_geometry(
                    gtype, verts, tx, ty, tol_half, bbox,
                ):
...
def _geometry_bounds(
    gtype: str, verts: list[tuple[float, float]],
...
def _rect_overlaps_ellipse(
    tx: float, ty: float, half: float,
    cx: float, cy: float, semi_a: float, semi_b: float,
) -> bool:
...
def _point_in_polygon(
    x: float, y: float, verts: list[tuple[float, float]],
) -> bool:
