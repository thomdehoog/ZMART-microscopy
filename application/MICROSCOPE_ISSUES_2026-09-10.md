...
## Validation 4: native operator checks on 2026-09-11

The repaired installation passed a bounded workflow in the actual native WebView2 operator window, using the controller's mock driver, one overview position, one focus point and two acquired targets. No real microscope acquisition or movement was performed. The user also watched the run and reported that it looked correct.

Evidence is saved locally under `C:\ProgramData\MinicondaZMB\home\t.de\operator-diagnostics\native-validation-4\run-1789083313114`: `report.json`, `bridge.log`, and screenshots `01-connected-mock.png` through `08-readable-during-rerun.png`.

- Focus produced 61 preview slices. Native focus navigation changed the displayed slice from Z00044 to Z00039. The sampled preview's mean maximum RGB component was 136/255, confirming image signal.
- The detection sidebar displayed image signal (mean maximum RGB component 145/255), and detection produced targets.
- Both target sidebar comparisons displayed while selecting the two acquired targets. Each selection took 84 ms in this small run. No `/api/targets/raise` request was sent. These observations are not a large-acquisition latency benchmark.
- Zooming the native canvas requested and successfully received three new finest-level Top chunks. This verifies fine-data loading in this case; it does not establish pixel-perfect rendering or exclude every resolution/cache defect.
- During a mock `Rerun current`, all 17 sampled requests for a previously committed fine chunk returned HTTP 200. Publication reported `preparing` and then `ready`; the Top source revision advanced from 2 to 3. The frontend retained the same viewer object.
- No JavaScript page errors were recorded. The broader request log included 118 HTTP 404 responses, including sparse/empty chunk requests; it was not an all-requests-success test. The monitored committed chunk remained readable throughout the rerun.

The checked-in `application/native-operator-validation.mjs` captures this workflow. From the repository root, use the operator environment's Node and Python, set `PYTHON` to that environment's Python executable and `OPERATOR_EVIDENCE_DIR` to an evidence folder, then run `node application/native-operator-validation.mjs`. First launch an idle, disposable native operator with the process-local WebView2 argument `--remote-debugging-port=9223`; port 8837 must be free for the isolated mock bridge. The script temporarily navigates the native page to that bridge and restores its original URL afterward. An optional `NATIVE_OPERATOR_URL` overrides the restoration URL. Mock configuration and acquired data are isolated from the normal session. The saved run used a pixel threshold of 10; the committed harness tightens it to 30 to exclude the dark canvas background. That stricter threshold has not been rerun end to end.

### Additional findings and remaining work

- **Long Windows output paths can prevent canonical store creation.** A focus capture under the deeper diagnostic output folder produced TIFFs but failed canonical conversion. A separate reproduction using the same source pixels failed with `FileNotFoundError` for the long temporary Zarr path and succeeded with a short temporary path. Evidence: `native-validation-4/path-limit.json` and `operator-diagnostics/verify_native_path_limit.py`. This is consistent with a Windows path-length limitation; repair path handling and preserve the underlying conversion error in operator feedback.
- **Focus failure feedback needs repair.** In that failed conversion run, the focus point was marked lost, but the backend's overall focus error was null and the native panel retained an Interrupt action after backend work stopped. The conversion exception is caught by `_keep_position_as_zarr` and stored as `zarr_error`; downstream focus reports missing canonical data. The exact cause of the stale native action has not yet been traced.
- **Carrier edits can consume the first navigation click.** Immediately clicking the next step after changing carrier height left Define Carrier active in two attempts. Blurring with Tab before navigation allowed the workflow to continue. The carrier blur handler calls synchronization that rebuilds the rail, a plausible cause requiring a focused regression and repair.
- Native navigation across multiple focus points, real-microscope operation, the original 20-target workload, and exhaustive native rendering/cache verification remain unvalidated. The successful small run narrows the earlier uncertainty; it does not close these larger investigations.
- The complete performance breakdown remains outstanding: store generation, composition, compression, I/O, cache reuse, previews, loading and rendering. Repairs 1–3 and this native check establish functional progress, not a complete performance diagnosis.
