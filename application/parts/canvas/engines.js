 ...
 * over HTTP if you want neuroglancer.** `python zmart-interface.py --built` does that
 * for you.
 *
...
export function whyOneIsMissing() {
  ...
  return (
    ...
    "`python zmart-interface.py --built` does that."
  );
}
  ...
  if (!load) {
    throw new Error(
      `there is no engine called "${name}" in this page. It was built with: ` +
        ...
        "parts/canvas/engines.js and build again.",
    );
  }
