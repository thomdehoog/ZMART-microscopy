 ...
 * reason, and the test beside this one is unusually easy to fool. Its middle step
 * makes two claims at once — position B is not on the screen, and position A
 * still is — and each of them can go quietly wrong in a different way.
 *
 ...
 * served. And "A is still on the screen" would be satisfied by a viewer that drew
 * whatever it found on disk and paid no attention to commits at all. So each
 * claim is checked here against a fault aimed squarely at it.
 *
 * **B is committed early.** During the very step that expects B hidden, B is
 * published — by the real publisher, which goes and inspects what is on disk and
 * moves the run's record only because what it finds justifies it. Not one byte of
 * image changes; the commit is the whole difference. The server then hands B's
 ...
 * whether B appears is decided by the commit alone.
 *
 * **The run refuses everything.** The server is made to behave as though nothing
 * had ever been published, so the whole screen goes black. B is still not drawn,
 * so the first claim still holds — and the claim that A is still there ought to
 * catch it. This is the one that proves the middle step is not quietly passing
 * over an empty screen, which is the failure that would be hardest to spot by
 * reading.
 *
 ...
 *
 * It takes about three minutes: one honest run to show the test is green to begin
 * with, and one run per fault. A fault that passes is the bad answer here, which
 * reads backwards, so the result is printed in words rather than left as an exit
 * code for somebody to interpret.
 */
